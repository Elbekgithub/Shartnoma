...
class Contract(models.Model):
    miqdor = models.DecimalField(max_digits=11, decimal_places=3)
    description = models.TextField()
    deadline = models.DateField(null=True, blank=True)
    session = models.ForeignKey(Session, on_delete=models.CASCADE, blank=True, null=True)
    is_current_shartnoma = models.BooleanField(default=False, blank=True, null=True) 

    def __str__(self):
        return str(self.miqdor)
...
class Group(models.Model):
    ...
    kurator = models.CharField(max_length=100, null=True, blank=True)
